...
function Loans() {
  ...
  useEffect(() => {
    loanService.getByStatus("COMPLETED");
      ...
      
    {
      ...
      selector: (row: any) => (
        ...
        <NavDropdown title="Action" id="basic-nav-dropdown">
            ...
            {(user?.role=="ADMIN" || user?.role=="ACCOUNTANT") && (row.status=="PROCEED" || row.status=="COMPLETED" ||  row.status=="PROCESSING" ||  row.status=="REJECTED" )?<>
            ...
            { row.status=="PROCEED" || row.status=="REJECTED"?             <NavDropdown.Item  href={`/loans/view/${row.id}?id=${row?.customer[0]?._id}`}>View</NavDropdown.Item>
:            <NavDropdown.Item  href={`/loans/edit/${row.id}?id=${row?.customer[0]?._id}`}>Edit</NavDropdown.Item>
}
            </>
    }
